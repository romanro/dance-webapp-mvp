...
exports.isAuthorized = (req, res, next) => {
  const provider = req.path.split('/')[1];
  ...
  if (token) {
    ...
    //       Yes, Quickbooks - We got nothing, redirect to res.redirect(`/oauth/${provider}`);
    //       No, Quickbooks and Google- refresh token and save, and then go to next();
    //    No:  Treat it like we got nothing, redirect to res.redirect(`/oauth/${provider}`);
  ...
  new FacebookStrategy(
    {
      ...
      callbackURL: `${process.env.BASE_URL}/oauth/facebook/callback`,
